...
In other words, while the pure propositions-as-types logic is ``constructive'' in the strong algorithmic sense mentioned above, the default $(-1)$-truncated logic is ``constructive'' in a different sense%
\footnote{The words ``constructive'' and ``intuitionistic'' have been used in contradictory ways by various mathematicians.
The propositions-as-types logic, traditional in type theory, is ``constructive'' in the strong sense that every proof \emph{is} itself a construction.
The $(-1)$-truncated logic matches instead the logic first formalized by Heyting under the name ``intutionistic'' (though the historical appropriateness of that name is questionable), and which is now used in other areas such as topos theory.
Neither includes (by default) the additional principles postulated by some mathematicians calling themselves ``constructive'' or ``intuitionist'', such as choice sequences or Markov's principle; these may also be freely added as axioms if desired, although some of them are incompatible with the axiom of excluded middle.};
